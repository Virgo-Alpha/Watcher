...
class ChangeDetectionService:
    """
    Service for detecting changes in scraped state and managing alerts.
    Implements alert mode logic (once vs on_change) and rate limiting.
    """

    def __init__(self, rate_limit_minutes: int = 60):
        """
        Initialize change detection service

        Args:
            rate_limit_minutes: Minimum minutes between alerts for same haunt (default: 60)
        """
    ...
    def should_alert(
        ...
        alert_mode: str,
        last_alert_state: Optional[Dict[str, Any]],
        current_state: Dict[str, Any],
        truthy_values: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
        Determine if an alert should be sent based on alert mode and state

        Args:
            has_changes: Whether changes were detected
            changes: Dictionary of changes
            alert_mode: Alert mode ('once' or 'on_change')
            last_alert_state: State when last alert was sent
            current_state: Current state
            truthy_values: Optional dictionary of truthy values for each key
        ...
        if alert_mode == 'on_change':
            # Alert on every change
            return True

        elif alert_mode == 'once':
            # Alert once when condition becomes truthy
            return self._should_alert_once(
                last_alert_state,
                current_state,
                truthy_values or {}
            )
            ...
            logger.warning(f"Unknown alert mode: {alert_mode}")
            return False

    def _should_alert_once(
        self,
        last_alert_state: Optional[Dict[str, Any]],
        current_state: Dict[str, Any],
        truthy_values: Dict[str, Any]
    ) -> bool:
        """
        Determine if alert should be sent in 'once' mode

        In 'once' mode, we alert when:
        1. No previous alert was sent (last_alert_state is None)
        2. Any tracked value becomes truthy (based on truthy_values config)

        Args:
            last_alert_state: State when last alert was sent
            current_state: Current state
            truthy_values: Dictionary mapping keys to lists of truthy values

        Returns:
            True if alert should be sent
        """
        if not last_alert_state:
            # No previous alert - check if any value is truthy
            return self._has_truthy_values(current_state, truthy_values)

        # Check if any value changed from non-truthy to truthy
        for key, truthy_list in truthy_values.items():
            old_value = last_alert_state.get(key)
            new_value = current_state.get(key)

            old_is_truthy = self._is_truthy(old_value, truthy_list)
            new_is_truthy = self._is_truthy(new_value, truthy_list)

            if not old_is_truthy and new_is_truthy:
                # Value became truthy - should alert
                return True

        return False

    def _has_truthy_values(
        self,
        state: Dict[str, Any],
        truthy_values: Dict[str, Any]
    ) -> bool:
        """
        Check if state has any truthy values
        ...
        for key, truthy_list in truthy_values.items():
            value = state.get(key)
            if self._is_truthy(value, truthy_list):
                return True
        return False

    def _is_truthy(self, value: Any, truthy_list: list) -> bool:
        """
        Check if a value is considered truthy based on truthy list

        Args:
            value: Value to check
            truthy_list: List of truthy values

        Returns:
            True if value is in truthy list (case-insensitive for strings)
        """
        if value is None:
            return False
        ...
        # Check against truthy list (case-insensitive)
        for truthy_value in truthy_list:
            if str(truthy_value).lower().strip() == value_str:
                return True
    ...
    def is_rate_limited(
        ...
        Returns:
            True if rate limited (too soon to send another alert)
        """
        ...
        min_time_between_alerts = timedelta(minutes=self.rate_limit_minutes)

        return time_since_last_alert < min_time_between_alerts

    def get_change_summary(self, changes: Dict[str, Any]) -> str:
        """
        ...
        # Multiple changes
        change_lines = []
        for key, change in list(changes.items())[:3]:  # Show first 3 changes
            old_val = change.get('old', 'None')
            new_val = change.get('new', 'None')
    ...
    def should_update_alert_state(
        self,
        alert_mode: str,
        alert_sent: bool
    ) -> bool:
        """
        Determine if last_alert_state should be updated
        ...
        Returns:
            True if last_alert_state should be updated
        """
        if alert_mode == 'once':
            # Update alert state when alert is sent in 'once' mode
            return alert_sent
        elif alert_mode == 'on_change':
            # Don't track alert state in 'on_change' mode
            return False
        else:
            return False
