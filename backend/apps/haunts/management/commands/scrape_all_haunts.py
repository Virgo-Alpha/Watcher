...
class Command(BaseCommand):
    help = 'Manually scrape all haunts in the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--haunt-id',
            type=str,
            help='Scrape only a specific haunt by ID',
        )
        parser.add_argument(
            '--active-only',
            action='store_true',
    ...
    def handle(self, *args, **options):
        haunt_id = options.get('haunt_id')
        active_only = options.get('active_only', False)
        
        self.stdout.write('=' * 80)
        self.stdout.write('Starting manual scrape of all haunts')
        self.stdout.write('=' * 80)
        
        # Get haunts
        if haunt_id:
            haunts = Haunt.objects.filter(id=haunt_id).select_related('owner', 'folder')
        else:
            haunts = Haunt.objects.all().select_related('owner', 'folder')
            if active_only:
                haunts = haunts.filter(is_active=True)
        
        total_haunts = haunts.count()
        
        self.stdout.write(f'\nFound {total_haunts} haunts in database\n')
        ...
        if changes_detected:
            ...
            for result in changes_detected:
                self.stdout.write(f'  - {result["name"]}: {result["changes_count"]} changes')
